...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val columnsToDrop = Array("Year", "Month", "DayofMonth", "DayOfWeek", "DepTime", "CRSDepTime", "UniqueCarrier", "FlightNum", "TailNum", "CRSElapsedTime", "Origin", "Dest", "Distance", "ArrTime", "ActualElapsedTime", "AirTime", "TaxiIn", "Diverted", "CarrierDelay", "WeatherDelay", "NASDelay", "SecurityDelay", "LateAircraftDelay")
    ...
    // Numerical columns for "mean" imputer
    var numCols = Array("CRSArrTime", "DepDelay", "TaxiOut")
    var columnsToDrop2 = df.columns
    for (i <- 0 until df.columns.length) {
      val column = df.columns(i)
      if (df.groupBy(column).count().groupBy(column).count().count() > 1) {
        columnsToDrop2 = columnsToDrop2.filter(_ != column)
      }
      else{
        if(numCols.contains(column)){ numCols = numCols.filter(_ != column) }
      }
    }
    ...
    imputer.setInputCols(numCols).setOutputCols(numCols).setStrategy("mean")
...
//      .setInputCol("CRSArrTime")
//      .setOutputCol("CRSArrTimeIndexed")
//    println("--------------------------------- Done -----------------------------------------------")
//    println()
//
//
...
//      .setInputCol("CRSArrTimeIndexed")
//      .setOutputCol("CRSArrTimeCat")
//    println("--------------------------------- Done -----------------------------------------------")
//    println()
    ...
    df = df.drop(numCols:_*).drop(Array("CRSArrTimeCat", "CRSArrTimeIndexed", "CRSArrTime"):_*)
